# A08 预推送守卫报告

- 时间：2026-03-04 18:14:10 CST
- 结论：failed
- 分支：main
- 提交：8bfd9cd

## 校验结果

| Check | Status |
| --- | --- |
| env_tracking | success |
| secret_scan | success |
| aiservice_health | failed |
| m11_smoke | failed |
| a04_smoke | blocked |
| m13_smoke | blocked |

## env 跟踪检查

```text
ok
```

## 密钥样式扫描

```text
ok
```

## aiservice 健康检查

- URL: http://host.docker.internal:8000/health

```text
curl: (6) Could not resolve host: host.docker.internal
```

## Git 工作区

```text
 M .env.example
 M .env.prod.example
 M README.md
 M docker-compose.prod.yml
 M "docs/\345\244\232\345\216\202\345\225\206AI\344\275\277\347\224\250\346\214\207\345\215\227.md"
 M scripts/m11_prod_down.sh
 M scripts/m11_prod_up.sh
 M scripts/m11_smoke.sh
 M src/main/java/com/bones/gateway/integration/ai/WebClientAiServiceClient.java
 M src/main/resources/application-dev.yml
 M src/main/resources/application-prod.yml
 M src/test/java/com/bones/gateway/integration/ai/WebClientAiServiceClientTest.java
?? "docs/A04-\347\275\221\345\205\263\344\270\216AI\346\234\215\345\212\241\347\273\237\344\270\200\347\274\226\346\216\222\344\272\244\344\273\230\350\257\264\346\230\216.md"
?? "docs/A05-AI\346\234\215\345\212\241\351\225\234\345\203\217\345\217\221\345\270\203\350\204\232\346\234\254\344\272\244\344\273\230\350\257\264\346\230\216.md"
?? "docs/A06-\351\230\266\346\256\265\351\252\214\346\224\266\344\270\216\346\243\200\346\237\245\347\202\271\346\212\245\345\221\212\350\257\264\346\230\216.md"
?? "docs/A07-\351\230\266\346\256\265\347\211\210\346\234\254\350\212\202\347\202\271\344\270\216\345\233\236\346\273\232\346\270\205\345\215\225\350\257\264\346\230\216.md"
?? "docs/A08-\351\242\204\346\216\250\351\200\201\345\256\210\345\215\253\344\270\216\346\225\217\346\204\237\344\277\241\346\201\257\346\211\253\346\217\217\350\257\264\346\230\216.md"
?? docs/reports/
?? scripts/a04_smoke.sh
?? scripts/a05_release_aiservice.sh
?? scripts/a06_checkpoint.sh
?? scripts/a07_stage_node.sh
?? scripts/a08_prepush_guard.sh
?? test.py
```

## m11_smoke 输出

```text
[M11-SMOKE] FRONTEND_URL=http://localhost:5173
[M11-SMOKE] GATEWAY_URL=http://localhost:8080
[1/3] gateway health
curl: (7) Failed to connect to localhost port 8080 after 0 ms: Couldn't connect to server
```

## a04_smoke 输出

```text
[A04-SMOKE] blocked: aiservice health check failed.
[A04-SMOKE] check: http://host.docker.internal:8000/health
```

## m13_smoke 输出

```text
[M13-SMOKE] blocked: aiservice health check failed.
[M13-SMOKE] check: http://host.docker.internal:8000/health
```

## 建议

1. 先修复 failed 项，再执行 push。
2. aiservice 当前不可达，先确认 ai-service 已启动并能被 gateway 访问。
3. 手动检查：`curl -fsS "http://host.docker.internal:8000/health" && echo ok`
4. 修复后重跑：`RUN_M13_SMOKE=true PROVIDER=aiservice MODEL=MiniMax-M2.5 ./scripts/a08_prepush_guard.sh`
5. 报告路径：`docs/reports/a08_prepush_20260304_181408.md`
